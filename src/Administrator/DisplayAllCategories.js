/* 
@material-table/core@next //for update material table 
@mui/lab
...
if(result.result){ Swal.fire({
  ...
  Swal.fire({
    ...
    const handleIconSave=async()=>{
      var formData = new FormData()
     formData.append('categoryid',categoryId)//****categoryname=req.body.categoryname in node***
     formData.append('icon',Icon.bytes)
     var result = await postImageData("categories/editcategoryicon",formData)
      if(result.result){ Swal.fire({
        position: 'top-end',
        icon: 'success',
    ...
    const handleDelete=async()=>{
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            var dresult = await postData("categories/deletecategory",{categoryid:categoryId})
      if(dresult.result)
            Swal.fire(
              'Deleted!',
    ...
    const showDialog=()=>{
      ...
      <Grid container spacing={2}>
          ...
          <Grid item xs={12}>
              <CssTextField container="outlined" value={categoryName} InputLabelProps={{style:{color:'#FFF'}}}  inputProps={{style:{color:'white'}}}label="Catagory" onChange={(event)=>setcategoryName(event.target.value)} fullWidth/>
          </Grid>
        ...
        return (
            <MaterialTable
                ...
                columns={[
                    ...
                    { title: 'Category Icon', field: 'icon' ,
